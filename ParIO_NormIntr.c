...
//void switch_handle();
void button_handle(); //有按键中断时读入开关状态
void timer_handle(); //有定时器中断时刷新数码管显示
...
int main()
{
    ...
    Xil_Out32(XPAR_INTC_0_BASEADDR + XIN_IAR_OFFSET,
			...
			|XPAR_AXI_TIMER_0_INTERRUPT_MASK); //清除GPIO2和Timer原中断状态
    Xil_Out32(XPAR_INTC_0_BASEADDR + XIN_IER_OFFSET,
    		XPAR_AXI_GPIO_2_IP2INTC_IRPT_MASK
			|XPAR_AXI_TIMER_0_INTERRUPT_MASK); //开这两位对应的中断引脚
    Xil_Out32(XPAR_INTC_0_BASEADDR + XIN_MER_OFFSET,
    		XIN_INT_MASTER_ENABLE_MASK
			|XIN_INT_HARDWARE_ENABLE_MASK); //使能总中断，即写HIE(D1)和ME(D0)为1使能硬件中断和允许产生中断请求信号
...
{
	int status;
	status=Xil_In32(XPAR_AXI_INTC_0_BASEADDR+XIN_ISR_OFFSET);//读入中断状态寄存器
	if((status&XPAR_AXI_GPIO_2_IP2INTC_IRPT_MASK)==XPAR_AXI_GPIO_2_IP2INTC_IRPT_MASK) //GPIO2的中断
		button_handle(); //读开关状态
	else if((status&XPAR_AXI_TIMER_0_INTERRUPT_MASK) == XPAR_AXI_TIMER_0_INTERRUPT_MASK) //Timer的中断
		timer_handle(); //在定时器发生中断时更新数码管
	Xil_Out32(XPAR_AXI_INTC_0_BASEADDR+XIN_IAR_OFFSET,status); //写IAR 清除中断状态寄存器ISR
}
...
//	Xil_Out16(XPAR_GPIO_0_BASEADDR + XGPIO_DATA2_OFFSET,swin);//led输出开关状态
	...
	//GPIO的IPISR寄存器，读表示读状态，写1代表清除状态，（相当于INTC的ISR和IARR）
	//IPISR不能自动清除，因此需要手动写程序清除（包括快速中断）
}
...
//	xil_printf("sw: %d\n", swin);
	...
	if(button==0x01)//BNTC读入16位数，将二进制低八位显示到数码管
	{
		...
		pos++;
		if(pos>=8) pos=0; //此处不能用==做判断条件，防止pos出现大于8（下边的大于4等）后不能跳转到0而一直加导致程序跑飞
	}
